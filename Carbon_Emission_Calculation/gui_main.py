...
try:
    ...
    print(f"WARNING: AI modules not found ({e}) - will use simulation mode")
    AI_AVAILABLE = False
...
class MockAI:
    ...
    def __init__(self):
        self.demo_foods = [
            ...
            ("Bread", 0.81), ("Cheese", 0.88), ("Eggs", 0.85),
            ("Blueberries", 0.90), ("Tomato", 0.87), ("Rice", 0.85)
        ]
    ...
    def detect_food(self, image_source):
        """Simulate detection for camera frame"""
        result = self.recognize_food("mock_path")
        # MockResult object has food_name, confidence
        return {
                        ...
                        if line.startswith("WEIGHT:"):
                            ...
                            if len(parts) >= 3:
                                try:
                                    ...
                                    if is_stable and weight > 50.0 and hasattr(self, 'root') and not self.is_analyzing:
                                        print(f"DEBUG: Auto-triggering analysis for stable weight {weight:.1f}g")
                                        # Trigger analysis in main thread if we had access to root (need to inject it)
                                        pass 
...
class FoodCarbonGUI:
    ...
    def __init__(self, root):
        ...
        if AI_AVAILABLE:
            try:
                ...
                self.ai_mode = "DEMO"
                # Fallback carbon calculator if module fails to init
                if 'CarbonCalculator' in globals():
                    self.carbon_calc = CarbonCalculator()
        else:
            self.vision_ai = MockAI()
            self.ai_mode = "DEMO"
    ...
    def create_widgets(self):
        ...
        # Third row: Camera control buttons
        ...
        # Right panel - Enhanced results panel (Fixed width)
        right_frame = tk.LabelFrame(content_frame, text="📊 Analysis Results & Controls", 
                                  font=('Arial', 12, 'bold'), bg='#3b3b3b', fg='white',
                                  relief='raised', borderwidth=2, padx=15, pady=15,
                                  width=450)  # Reduce fixed width
        right_frame.pack(side=tk.RIGHT, fill=tk.Y, padx=(10, 0))
        right_frame.pack_propagate(False)  # Prevent child widgets from changing size
        ...
        self.ai_result_text = tk.Text(ai_frame, height=3, font=('Consolas', 9),  # Reduce height and font
        ...
        self.weight_text = tk.Text(weight_frame, height=2, font=('Consolas', 9),  # Reduce height
        ...
        self.carbon_text = scrolledtext.ScrolledText(carbon_frame, height=15, font=('Consolas', 8),  # Reduce font
    ...
    def start_camera(self):
        ...
        try:
            ...
            camera_indices = [0, 1]  # Try standard then external
    ...
    def _analyze_worker(self):
        ...
        try:
            ...
            self.current_detection = {
                ...
                'standard_name': ai_result.food_name.lower().strip(),
    ...
    def _analyze_frame_worker(self, frame, weight_grams):
        ...
        try:
            ...
            if not detection_result or detection_result.get('standard_name') == 'Recognition Failed':
    ...
    def send_ai_results_to_hardware(self, carbon_result, weight_grams):
        ...
        try:
            ...
            impact_level = carbon_result.get('impact_level', 'UNKNOWN')
    ...
    def send_to_display(self):
        ...
        try:
            ...
            # Get the data from final results (flat structure)
            detection_result = self.final_results
            carbon_result = self.final_results['carbon_result']
    ...
    def display_final_results(self):
        ...
        if not carbon.get('in_database', True):
            ...
            result_text += f"├─ Default Factor: Using {self.carbon_calc.default_factor} kg CO₂/kg estimate\n"
